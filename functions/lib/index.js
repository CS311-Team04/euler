...
exports.getEpflScheduleStatusFn = exports.disconnectEpflScheduleFn = exports.syncEpflScheduleFn = exports.getScheduleContextCore = exports.syncEpflScheduleCore = exports.moodleListCoursesFn = exports.moodleSearchFileFn = exports.connectorsMoodleTestFn = exports.connectorsMoodleDisconnectFn = exports.connectorsMoodleConnectFn = exports.connectorsMoodleFetchTokenFn = exports.connectorsMoodleStatusFn = exports.edConnectorTestFn = exports.edConnectorDisconnectFn = exports.edConnectorConnectFn = exports.edConnectorStatusFn = exports.generateTitleFn = exports.answerWithRagHttp = exports.indexChunksHttp = exports.answerWithRagFn = exports.indexChunksFn = exports.ping = exports.onMessageCreate = exports.answerWithRagCore = exports.indexChunksCore = exports.generateTitleCore = exports.apertusChatFnCore = exports.setChatClient = void 0;
...
const moodleIntent_1 = require("./connectors/moodle/moodleIntent");
...
exports.answerWithRagFn = europeFunctions.https.onCall(async (data, context) => {
    try {
        ...
        if (uid) {
            const moodleIntentResult = (0, moodleIntent_1.detectMoodleIntent)(question);
            if (moodleIntentResult.moodle_intent_detected && moodleIntentResult.moodle_intent === "get_file") {
                ...
                if (moodleIntentResult.course_query) {
                    const searchResult = await searchMoodleFileCore(uid, moodleIntentResult.course_query, moodleIntentResult.file_query || "lecture");
                ...
                try {
                    const token = (0, secretCrypto_1.decryptSecret)(config.tokenEncrypted);
                    const client = new MoodleClient_1.MoodleClient(config.baseUrl, token);
 ...
 */
async function searchMoodleFileCore(uid, courseQuery, fileQuery) {
 ...
 */
exports.moodleSearchFileFn = europeFunctions.https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError("unauthenticated", "User must be authenticated");
    }
    ...
    if (!courseQuery) {
        throw new functions.https.HttpsError("invalid-argument", "courseQuery is required");
    }
    try {
        return await searchMoodleFileCore(context.auth.uid, courseQuery, fileQuery || "lecture");
    }
    catch (e) {
        logger.error("moodleSearchFileFn.failed", { error: String(e), uid: context.auth.uid });
        throw new functions.https.HttpsError("internal", e.message || "Failed to search for file");
    }
});
/**
 * Lists all courses the user is enrolled in on Moodle.
 */
exports.moodleListCoursesFn = europeFunctions.https.onCall(async (_data, context) => {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError("unauthenticated", "User must be authenticated");
    }
    const uid = context.auth.uid;
