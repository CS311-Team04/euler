...
class AuthStateTest {
  ...
  fun `Error should support long message`() {
    val longMessage =
        "This is a very long error message that contains multiple words and should be properly handled by the AuthState.Error data class"
    ...
    assertTrue(
        "Long error message should contain original text",
        longError.toString().contains(longMessage))
  }
  ...
  fun `Error instances with same message should be equal`() {
    ...
    assertEquals(
        "Error instances with same message should have same hash",
        error1.hashCode(),
        error2.hashCode())
  }
  ...
  fun `Error instances with different messages should not be equal`() {
    ...
    assertNotEquals(
        "Error instances with different messages should have different hash",
        error1.hashCode(),
        error2.hashCode())
  }
  ...
  fun `All singleton states should be equal to themselves`() {
    assertEquals(
        "NotInitialized should equal itself", AuthState.NotInitialized, AuthState.NotInitialized)
  ...
  fun `Different singleton states should not be equal`() {
    assertNotEquals(
        "NotInitialized should not equal Ready", AuthState.NotInitialized, AuthState.Ready)
    assertNotEquals(
        "NotInitialized should not equal SigningIn", AuthState.NotInitialized, AuthState.SigningIn)
    assertNotEquals(
        "NotInitialized should not equal SignedIn", AuthState.NotInitialized, AuthState.SignedIn)
  ...
  fun `AuthState should work in when expressions`() {
    fun getStateDescription(state: AuthState): String =
        when (state) {
    ...
    assertEquals(
        ...
        "Not initialized",
        getStateDescription(AuthState.NotInitialized))
    ...
    assertEquals(
        "Error description",
        "Error: Test error",
        getStateDescription(AuthState.Error("Test error")))
  }
