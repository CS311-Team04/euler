...
class SimpleAuthTest {
  ...
  fun `test that auth states are defined correctly`() {
    ...
    assertTrue(
        "AuthState.NotInitialized doit exister",
        AuthState.NotInitialized is AuthState.NotInitialized)
  }

  @Test
  fun `test that user model is correct`() {
    val user =
        User(username = "test@example.com", accessToken = "mock_token", tenantId = "mock_tenant")
