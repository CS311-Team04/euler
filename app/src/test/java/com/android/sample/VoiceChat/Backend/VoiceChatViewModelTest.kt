...
class VoiceChatViewModelTest {
  ...
  fun handleUserUtterance_creates_conversation_when_signed_in() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          ...
          whenever(repo.appendMessage(any(), any(), any())).thenReturn("msg-id-1")
        }
  ...
  fun handleUserUtterance_persists_user_message() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          ...
          whenever(repo.appendMessage(any(), any(), any())).thenReturn("msg-id-2")
        }
  ...
  fun handleUserUtterance_persists_ai_message() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          ...
          whenever(repo.appendMessage(any(), any(), any())).thenReturn("msg-id-3")
        }
  ...
  fun handleUserUtterance_creates_title_from_first_message() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          ...
          whenever(repo.appendMessage(any(), any(), any())).thenReturn("msg-id-4")
        }
  ...
  fun handleUserUtterance_reuses_existing_conversation() =
      runTest(testDispatcher) {
        ...
        runBlocking { whenever(repo.appendMessage(any(), any(), any())).thenReturn("msg-id-5") }
  ...
  fun handleUserUtterance_persist_user_message_handles_error() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          ...
          whenever(repo.appendMessage(eq("conv-id"), eq("assistant"), any())).thenReturn("msg-id-6")
        }
  ...
  fun handleUserUtterance_persist_ai_message_handles_error() =
      runTest(testDispatcher) {
        ...
        runBlocking {
          whenever(repo.startNewConversation(any())).thenReturn("conv-id")
          whenever(repo.appendMessage(eq("conv-id"), eq("user"), eq("Test"))).thenReturn("msg-id-7")
