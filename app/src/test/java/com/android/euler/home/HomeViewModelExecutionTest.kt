...
class HomeViewModelExecutionTest {
    ...
    fun `updateMessageDraft should update message draft correctly`() = runTest {
        ...
        assertEquals(
            "Message draft should be updated again",
            anotherMessage,
            afterSecondUpdate.messageDraft
        )
        ...
        val longMessage =
            "This is a very long message to test that the ViewModel can handle long text inputs properly without any issues."
        viewModel.updateMessageDraft(longMessage)
        val afterLongUpdate = viewModel.uiState.first()
        assertEquals(
            "Message draft should handle long text",
            longMessage,
            afterLongUpdate.messageDraft
        )
    }
    ...
    fun `sendMessage should add message to recent actions and clear draft`() = runTest {
        ...
        assertEquals(
            "Recent actions count should increase",
            initialRecentCount + 1,
            afterSend.recent.size
        )
        ...
        assertTrue(
            "New action should contain 'Messaged EULER'",
            newAction.title.contains("Messaged EULER")
        )
    ...
    fun `sendMessage should not add action when draft is empty`() = runTest {
        ...
        assertEquals(
            "Recent actions count should remain the same",
            initialRecentCount,
            afterSend.recent.size
        )
    ...
    fun `sendMessage should trim whitespace from message`() = runTest {
        ...
        val newAction = afterSend.recent.first()
        assertTrue(
            "New action should contain trimmed message",
            newAction.title.contains("Hello EULER!")
        )
        assertFalse(
            "New action should not contain leading/trailing spaces",
            newAction.title.contains("  Hello EULER!  ")
        )
    }
    ...
    fun `toggleSystemConnection should toggle system connection status`() = runTest {
        ...
        assertEquals(
            "Connection status should be toggled",
            !initialConnectionStatus,
            toggledSystem!!.isConnected
        )
        ...
        assertEquals(
            "Connection status should be back to original",
            initialConnectionStatus,
            backToOriginalSystem!!.isConnected
        )
    }
    ...
    fun `toggleSystemConnection should not affect other systems`() = runTest {
        ...
        assertEquals(
            "System 1 status should be toggled",
            !system1InitialStatus,
            system1After!!.isConnected
        )
        assertEquals(
            "System 2 status should remain unchanged",
            system2InitialStatus,
            system2After!!.isConnected
        )
    }
    ...
    fun `toggleSystemConnection should handle non-existent system gracefully`() = runTest {
        ...
        assertEquals(
            "Systems list should remain unchanged",
            initialSystems.size,
            afterToggle.systems.size
        )
        ...
        initialSystems.forEach { originalSystem ->
            ...
            assertEquals(
                "System status should be unchanged",
                originalSystem.isConnected,
                afterSystem!!.isConnected
            )
        }
    }
    ...
    fun `sendMessage with multiple messages should maintain chronological order`() = runTest {
        ...
        assertTrue(
            "First action should be the most recent",
            firstAction.title.contains("Third message")
        )
        assertTrue(
            "Second action should be the second most recent",
            secondAction.title.contains("Second message")
        )
    ...
    fun `state should be immutable - changes should not affect previous state`() = runTest {
        ...
        // Note: En réalité, StateFlow partage la même référence, mais le test vérifie le
        // comportement attendu
        assertNotEquals(
            "New state should be different from initial",
            initialMessageDraft,
            afterUpdate.messageDraft
        )
        assertEquals(
            "New state should have the updated message",
            "New message",
            afterUpdate.messageDraft
        )
    }
