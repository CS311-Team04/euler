...
class ElevenLabs_LiveTTSClient(
   ...
   *
   * The call is idempotent: repeated invocations while connected are ignored. Returns the resulting
   * [SessionState] immediately (use [state] flow to observe transitions).
   */
   ...
   * @param flush when `true`, instructs the server to cancel any previous synthesis and start
   *   fresh.
  ...
  companion object {
    ...
    private const val DEFAULT_BASE_URL = "wss://api.elevenlabs.io/v1/text-to-speech"
