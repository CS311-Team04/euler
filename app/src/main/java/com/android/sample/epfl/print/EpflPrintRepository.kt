...
class EpflPrintRepository(
  ...
  object OAuthConfig {
    ...
    const val REDIRECT_URI =
        "https://link.pocketcampus.org/pocketcampus:WEB/authentication/callback"
  ...
  /**
   * Get access token, refreshing if needed This is a suspend function that will attempt to refresh
  ...
  suspend fun refreshTokens(refreshToken: String? = null): String? {
    ...
    return try {
      val data = hashMapOf("refreshToken" to tokenToUse)

      val result = functions.getHttpsCallable("epflPrintRefreshTokenFn").call(data).await()
      ...
      if (success && response != null) {
        ...
        // Store new tokens
        storeTokens(accessToken, newRefreshToken ?: tokenToUse, expiresIn, getConnectedEmail())
   ...
   *
   * The Cloud Function handles the actual token exchange with Microsoft, so we don't expose client
   * secrets in the app.
   */
  suspend fun exchangeCodeForTokens(authCode: String): TokenExchangeResult {
    ...
    return try {
      ...
      val data = hashMapOf("authCode" to authCode, "redirectUri" to OAuthConfig.REDIRECT_URI)

      val result = functions.getHttpsCallable("epflPrintExchangeTokenFn").call(data).await()
  ...
  suspend fun submitPrintJob(
    ...
    return try {
      ...
      val result = functions.getHttpsCallable("epflPrintSubmitJobFn").call(data).await()
  ...
  suspend fun getCreditBalance(): CreditBalanceResult {
    ...
    return try {
      ...
      val result = functions.getHttpsCallable("epflPrintGetBalanceFn").call(data).await()
