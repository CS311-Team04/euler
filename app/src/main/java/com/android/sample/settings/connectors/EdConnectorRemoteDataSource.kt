...
/**
 * Petit data source pour appeler les Cloud Functions ED.
 * Chaque méthode renvoie la config actuelle renvoyée par le backend.
 */
class EdConnectorRemoteDataSource(
    ...
    suspend fun disconnect(): EdConnectorConfigRemote {
        val result = functions
            .getHttpsCallable(FN_DISCONNECT)
            .call()
            .await()
        return mapEdConnectorConfig(result.getData())
    }
